...
# Validamos si gitlab existe como remote antes de hacer push
$remotes = git remote
if ($remotes -contains "gitlab") {
    git pull gitlab main --rebase
    git push gitlab main
} else {
    Write-Host "[!] Remote 'gitlab' no configurado, omitiendo volcado a GitLab." -ForegroundColor Yellow
}
...
git rm --cached .gitlab-ci.yml -q -f 2>$null
...
Write-Host "[*] Portafolio público en GitHub actualizado y entorno GitLab protegido." -ForegroundColor Green
